...
# 1. Generate ruleset
python3 "$HERE/projector.py" --dna "$DNA_PATH" --conventions "$CONV_PATH" --out "$PROJECT_ROOT/.agent/tools/rule-projector/generated"
python3 "$HERE/backends/checkstyle.py" --ir "$PROJECT_ROOT/.agent/tools/rule-projector/generated/rules.json" --out "$RULESET_PATH"
