...
- Biến 1 (hoặc nhiều) tài liệu dạng tự do (wiki, Confluence, PRD, SRS, ghi chú…) thành **khối yêu cầu có cấu trúc** trong `.knowledge-layer/active/REQUIREMENT.md`.
...
Cập nhật `.knowledge-layer/active/REQUIREMENT.md`:
...
1. Nếu `.knowledge-layer/active/REQUIREMENT.md` **chưa có**:
...
2. Nếu `.knowledge-layer/active/REQUIREMENT.md` **đã có** (ví dụ sau `requirement-analyst`):
