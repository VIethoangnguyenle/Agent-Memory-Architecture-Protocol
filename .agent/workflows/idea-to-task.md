...
- Đã có file `ideation-*.md` trong `.knowledge-layer/active/ideation/` với phần **“Đề xuất scope task”** và AC gợi ý tương đối rõ.
...
- Cập nhật `.knowledge-layer/active/AGENT_TRANSPARENCY.md`:
...
1. Nếu user cung cấp tên/slug:
   - Tìm file khớp nhất trong `.knowledge-layer/active/ideation/`.
...
FUNCTION check_ideation_expiry():
  FOR EACH file IN .knowledge-layer/active/ideation/ideation-*.md:
...
FUNCTION auto_archive_expired_ideation(filename):
  IF user chọn [A] hoặc không phản hồi sau 30 ngày thêm:
    1. Đọc file ideation
    2. Append vào .knowledge-layer/archive/ARCHIVE_LOG.md:
       | {filename} | {created_at} | {ideation_expiry} | expired | {tóm tắt 1 câu} |
    3. Xoá file khỏi .knowledge-layer/active/ideation/
