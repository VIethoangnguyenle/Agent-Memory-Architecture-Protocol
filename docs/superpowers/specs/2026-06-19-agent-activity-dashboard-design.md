...
### Phân tầng nguồn (đã chốt: dùng A+B, defer C)

| Tầng | Nguồn | Cho gì | Neutral | Quyết định |
|------|-------|--------|---------|-----------|
| **A** | 3 contract files | phase, x/N task, token-ước-tính | ✅ | **dùng** — backbone |
| **B** | activity hook (P5) | tool-call realtime + tóm tắt tham số | ✅ (deterministic, không lệ thuộc LLM ghi) | **dùng** — tín hiệu realtime chính |
| **C** | brain native của platform (Claude `~/.claude/projects/*.jsonl`, Codex `~/.codex/`, Antigravity `state.vscdb`) | token **chính xác** + full I/O | ❌ mỗi platform một kiểu | **defer** (xem §9) |

Lý do defer C: bất đối xứng nặng. Claude/Codex có session JSONL sạch (token thật, tool_use đầy
đủ), nhưng Antigravity chỉ có SQLite `state.vscdb` (key-value VS Code, schema không tài liệu,
đổi theo version) → reverse-engineer fragile, phá vỡ neutral. Tầng B (hook) đã cho "đang gọi
tool nào" một cách deterministic và chạy được trên Antigravity qua `hooks.json` sẵn có, nên
**không cần** phụ thuộc C để đạt mục tiêu.
...
| **P2.5** | **Validation gate** (Antigravity, subagent ngầm): test **cả hai** — (a) contract files cập nhật live (mtime đổi mid-run); (b) activity hook (B) có fire trong subagent ngầm không | quan sát được tín hiệu realtime qua A hoặc B |
...
- **Tầng C (brain native)** — đọc session store riêng từng platform. Enrichment tương lai
  cho Claude/Codex (token chính xác + full I/O qua JSONL sạch) nếu cần; **không** phụ thuộc
  `state.vscdb` của Antigravity (fragile, undocumented, breaks neutral).
