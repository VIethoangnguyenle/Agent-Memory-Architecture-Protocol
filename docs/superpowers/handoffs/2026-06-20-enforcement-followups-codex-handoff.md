...
> enforcement audit of Maika's RULES/SKILLS/WORKFLOWS layer. #1–#3 are already shipped.

---

## 0. Context you need first

Maika is an agent framework whose source lives under `.maika/` (rules, skills, workflows,
...
hook** (`.maika/hooks/write-gate/write_gate.py`, a PreToolUse hook) enforces anything
...
**Process expectation:** Maika work goes brainstorm → spec (`docs/superpowers/specs/`) →
...
**Problem.** `R-Guard-1` ([.maika/rules/rules-guard.md](../../../.maika/rules/rules-guard.md))
...
- **knowledge-curator** ([.maika/skills/knowledge-curator/SKILL.md](../../../.maika/skills/knowledge-curator/SKILL.md))
...
- Overall: 8/14 skills have no `pre_conditions`; **0/14 declare `outputs:`** even though
  `R-Path-1` ([.maika/rules/rules-knowledge.md](../../../.maika/rules/rules-knowledge.md) §12)
  has a full "who writes what" table.

**Verify current state:** `/usr/bin/grep -rL "pre_conditions" .maika/skills/*/SKILL.md`.
Schema + lint live in `.maika/tools/skill-lint/validate_skills.py` (frontmatter `F4`
...
1. Add a `pre_conditions:` block to **knowledge-curator** that encodes its existing prose
   ...
   Confirm `skill-lint` still passes (`/usr/bin/python3 .maika/tools/skill-lint/validate_skills.py`).
...
`.maika/tools/skill-lint/tests/` if you change the schema.
...
**Problem.** `R-DNA-7` ([.maika/rules/rules-guard.md](../../../.maika/rules/rules-guard.md))
...
Verify: `/usr/bin/grep -rln "teaching moment\|R-DNA-7" .maika/skills .maika/workflows .maika/procedures`
...
at `maika init` (see `R-Adapter-1` in [.maika/rules/rules-tool.md](../../../.maika/rules/rules-tool.md)
§R-Tool-7, and usages in `task.md`). But the DB tool is hard-coded as the literal `db-remote`:
- [.maika/skills/db-explorer/SKILL.md](../../../.maika/skills/db-explorer/SKILL.md) (e.g. lines ~5, ~72, ~80, ~149, ~311)
- [.maika/skills/codebase-explorer/SKILL.md](../../../.maika/skills/codebase-explorer/SKILL.md) (~226, ~232)
Verify: `/usr/bin/grep -rn "db-remote" .maika/skills`.
...
may be cleanest as the first concrete slice of P2.1. Keep Maika generic: no project-specific
DB names hard-coded (see memory: "Maika framework generic boundary").

**6b — orphan skill `document-writer`.**
`document-writer` lints PASS but **no workflow references it**. Verify:
`/usr/bin/grep -rn "document-writer" .maika/workflows` (expect nothing).
