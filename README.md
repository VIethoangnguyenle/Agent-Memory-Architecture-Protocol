...
│
└── .amap/                             ← Toàn bộ framework + bộ nhớ (một dotdir duy nhất)
...
> 🔌 Với Claude Code, Codex CLI và Antigravity, `amap init` còn mirror mỗi skill/workflow
> vào đúng cơ chế native discovery của tool đó (`.claude/skills/`, `.agents/skills/`) —
> xem [Tương thích với các AI Agent](#tương-thích-với-các-ai-agent) bên dưới.
...
1. **Platform** — Antigravity, Claude Code, Cursor, Codex CLI, hoặc Generic
...
AMAP hoạt động với bất kỳ AI coding agent nào có thể đọc file dự án. 4 platform có
adapter riêng trong `amap init` — chọn đúng platform khi init, CLI tự render entry
point đúng tên file **và** mirror mỗi skill/workflow vào đúng cơ chế native discovery
của tool đó (skill `requirement-analyst` xuất hiện thẳng trong danh sách skill của
Claude Code/Codex, không chỉ qua agent tự đọc `bootstrap.md`). Platform không có adapter
riêng (`generic`) vẫn hoạt động đầy đủ qua `AGENTS.md` + đọc prose — chỉ thiếu phần
native discovery.

| AI Tool | Platform trong `amap init` | Entry Point | Native skill discovery |
|---------|------------------------------|--------------|--------------------------|
| **Claude Code** | `claude-code` | `CLAUDE.md` | ✅ `.claude/skills/` |
| **Codex CLI (OpenAI)** | `codex` | `AGENTS.md` | ✅ `.agents/skills/` |
| **Google Antigravity** | `antigravity` | `AGENTS.md` (User Rules) | ✅ `.agents/skills/` (chuẩn mở agentskills.io — chung với Codex) |
| **Cursor** | `cursor` | `.cursorrules` | ✅ `.cursor/commands/` (gọi thủ công, Cursor không auto-trigger theo description) |
| **Gemini CLI / Jules** | `generic` | `AGENTS.md` (native) | — agent tự đọc qua `bootstrap.md` |
| **GitHub Copilot** | `generic` | `.github/copilot-instructions.md` (tạo file trỏ về `AGENTS.md`) | — agent tự đọc qua `bootstrap.md` |
| **Windsurf** | `generic` | `.windsurfrules` (tạo file trỏ về `AGENTS.md`) | — agent tự đọc qua `bootstrap.md` |
...
Có. AMAP là tầng protocol — không chứa code ứng dụng. Copy `.amap/` (đã gồm `knowledge/`) vào repo private của bạn rồi tuỳ chỉnh `knowledge-snapshot.md` theo kiến trúc hệ thống của bạn.
